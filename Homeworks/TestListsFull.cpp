...
bool fillArray(const std::string &fname,list<V> &data) {
	T lst(fname);
	lst.clear();
...
template<typename T,typename V>
bool checkEquality(const std::string &fname,list<V> &data) {
	T lst(fname);
	if(lst.size() != static_cast<int>(data.size())) return false;
	auto iter = lst.begin();
	for(V &v:data) {
...
int main(int argc,char **argv) {
	...
	FileLinkedList<int> al1("al1.bin");
	if(!simpleTest(al1,"alst.bin")) return 2;
	cout << "LinkedList passed simple." << endl;

	list<Data> v;
	if(!fillArray<FileLinkedList<Data>,Data>("data.bin",v)) {
		cout << "LinkedList failed on fill." << endl;
		return 2;
	}
	if(!checkEquality<FileLinkedList<Data>,Data>("data.bin",v)) {
		cout << "LinkedList failed equality test." << endl;
		return 2;
	}
	v.clear();
	...
	FileLinkedList<int> al3("al3.bin");
	al3.clear();
	list<int> ll3;
	ll3.clear();
	for(int i=0; i<n; ++i) {
...
//		if(i%100==0) cout << i << " " << al3.size() << endl;
	...
	auto ll3iter = ll3.begin();
	for(auto al3iter = al3.begin(); al3iter!=al3.end(); ++al3iter, ++ll3iter) {
		al3.set(*al3iter+1,al3iter);
		++(*ll3iter);
	}
	ll3iter = ll3.begin();
	for(auto al3iter = al3.begin(); al3iter!=al3.end(); ++al3iter, ++ll3iter) {
		if(*al3iter != *ll3iter) {
			cout << "Failed on set of iterator.\n";
			return 2;
		}
	}
